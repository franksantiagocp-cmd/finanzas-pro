    ...
    tb.innerHTML=rows.length
      ? rows.map(r=>`<tr>
          ...
          <td style="white-space:nowrap">
            <button class="btn btn-sm btn-ico" onclick='openEditIng(${JSON.stringify(r)})' title="Editar" style="margin-right:4px">✎</button>
            <button class="btn btn-d btn-ico btn-sm" onclick="delIng(${r.id})" title="Eliminar">✕</button>
          </td>
        </tr>`).join('')
...
async function renderSub(sub) {
  ...
  if(tb) tb.innerHTML=gastos.length
    ? gastos.map(r=>`<tr>
        ...
        <td style="white-space:nowrap">
          <button class="btn btn-sm btn-ico" onclick='openEditGas(${JSON.stringify(r)})' title="Editar" style="margin-right:4px">✎</button>
          <button class="btn btn-d btn-ico btn-sm" onclick="delGas(${r.id},'${sub}')" title="Eliminar">✕</button>
        </td>
      </tr>`).join('')
...
/* ── EDICIÓN INGRESOS ────────────────────────────────────────────────── */
function openEditIng(r) {
  $('edit-ing-id').value    = r.id;
  $('edit-ing-fecha').value = (r.fecha+'').split('T')[0];
  $('edit-ing-monto').value = r.monto;
  $('edit-ing-desc').value  = r.descripcion || '';

  // Poblar select con categorías y preseleccionar la actual
  const sel = $('edit-ing-cat');
  if (sel && S.cfg?.catIngresos) {
    sel.innerHTML = S.cfg.catIngresos
      .map(c => `<option value="${c.id}" ${c.nombre===r.categoria?'selected':''}>${c.nombre}</option>`)
      .join('');
  }
  openM('m-edit-ing');
}
...
async function guardarEditIng() {
  const id       = $('edit-ing-id').value;
  const fecha    = $('edit-ing-fecha').value;
  const cat_id   = $('edit-ing-cat').value;
  const monto    = parseFloat($('edit-ing-monto').value);
  const desc     = $('edit-ing-desc').value.trim();
  ...
  try {
    await api('PUT', `/api/ingresos/${id}`, {
      fecha, categoria_id: parseInt(cat_id), monto, descripcion: desc
    });
    closeM('m-edit-ing');
    await renderTablaIng();
    await renderInicio();
  } catch(e) { alert(e.message); }
}

/* ── EDICIÓN GASTOS ──────────────────────────────────────────────────── */
function openEditGas(r) {
  $('edit-gas-id').value    = r.id;
  $('edit-gas-sub').value   = r.subcuenta;
  $('edit-gas-fecha').value = (r.fecha+'').split('T')[0];
  $('edit-gas-monto').value = r.monto;
  $('edit-gas-desc').value  = r.descripcion || '';
  $('m-edit-gas-title').textContent = `Editar gasto — ${SUB_META[r.subcuenta]?.label||r.subcuenta}`;

  const sel = $('edit-gas-cat');
  if (sel && S.cfg?.catGastos) {
    sel.innerHTML = S.cfg.catGastos
      .map(c => `<option value="${c.nombre}" ${c.nombre===r.categoria?'selected':''}>${c.nombre}</option>`)
      .join('');
  }
  openM('m-edit-gas');
}

async function guardarEditGas() {
  const id    = $('edit-gas-id').value;
  const sub   = $('edit-gas-sub').value;
  const fecha = $('edit-gas-fecha').value;
  const cat   = $('edit-gas-cat').value;
  const monto = parseFloat($('edit-gas-monto').value);
  const desc  = $('edit-gas-desc').value.trim();

  if (!fecha)          { alert('Selecciona una fecha.'); return; }
  if (!monto||monto<=0){ alert('Ingresa un monto válido.'); return; }

  try {
    await api('PUT', `/api/gastos/${id}`, {
      fecha, categoria: cat, descripcion: desc, monto
    });
    closeM('m-edit-gas');
    await renderSub(sub);
    await renderInicio();
